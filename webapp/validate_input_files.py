...
class FileValidation:
    """has the FileSystem and methods to validate coldata and counts files"""
    ...
    def error_message(self, df, file):
        ...
        if file == 'rna_seq_col':
            x = self.file_obj._rna_seq_col_file['format']
        elif file == 'rna_seq_count':
            x = self.file_obj._rna_seq_count_file['format']
        elif file == 'microarray_col':
            x = self.file_obj._micro_array_col_file['format']
        elif file == 'microarray_count':
            x = self.file_obj._micro_array_count_file['format']
        for k in x.keys():
            if x[k] == type(0.0):
                x[k] = 'float64'
            elif x[k] == type(0):
    ...
    def validate_file(self, col_path, count_path):
        ...
        Take file paths of col and count files, returns error message"""
