...
def append_list_as_row(file_name, header, data):
    file_path = f"webapp/tests/{file_name}"
    with open(file_path, 'w', encoding='UTF8', newline='') as write_obj:
...
class TestInput(unittest.TestCase):
    ...
    def test1(self):
        ...
        self.col = f"tests/{self.col}"
        self.count = f"tests/{self.count}"
        self.assertEqual(validate.validate_file(self.col, self.count), "")
    ...
    def test2(self):
        ...
        self.assertEqual(validate.validate_file(self.col, self.count), expected)
